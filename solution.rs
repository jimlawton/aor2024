...
#[derive(Debug)]
struct UnwrappedError(String);

impl fmt::Display for UnwrappedError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "There is an error: {}", self.0)
    }
}

impl Error for UnwrappedError {}
...
impl Santa {
        ...
        G: Gift,
    {
        ...
        if !gift.is_wrapped() {
            return Err(Box::new(UnwrappedError("Gift is not wrapped".into())));
        }
